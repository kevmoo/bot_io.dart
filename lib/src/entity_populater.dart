...
abstract class EntityPopulater {
  static Future<FileSystemEntity> populate(String path, dynamic source,
    ...
    if (source is Stream) {
      ...
      return _ensurePath(path,
              ...
              overwriteExisting: overwriteExisting,
              leaveExistingDir: leaveExistingDirs)
          .then((_) => _populateDirWithMap(
              path, source, overwriteExisting, leaveExistingDirs));
    }

    throw new Exception("Cannot populate from '$source'.");
  }

  static Future<Directory> updateDirectory(
      String path, Map<String, dynamic> source) {
    assert(path != null);
    assert(source != null);

    var existingDir = new Directory(path);

    return existingDir.exists().then((bool exists) {
      if (!exists) {
        throw new EntityPopulatorException._internal(
            'Expected directory to'
            ' exist at $path',
            path);
      }
   ...
   */
  static Future<File> _populateFileWithStream(
      String path, Stream<List<int>> content) {
    var file = new File(path);

    return content.pipe(file.openWrite()).then((_) => file);
  }

  /**
   * We assume [_ensurePath] has been called first.
   */
  static Future<Directory> _populateDirWithMap(
      String path,
    ...
    return dir.create(recursive: false).then((_) {
  ...
  static Future _ensurePath(String path,
    ...
    return parentDir.exists().then((bool parentDirExists) {
